...
object ProcessFunctionTest {
  def main(args: Array[String]): Unit = {
    ...
    val processedStream1: DataStream[String] = dataStream.keyBy(_.id)
    ...
    val processedStream3: DataStream[(String, Double, Double)] = dataStream.keyBy(_.id)
      .flatMapWithState[(String,Double,Double),Double]{
          //如果没有状态的话，也就是没有数据来过，那么就将当前的数据温度值存入状态
          case (input: SensorReading,None) => (List.empty,Some(input.temperature))
          //如果有状态，就应该与上次的温度比较差值，如果大于阈值就输出报警
...
class TempChangeAlert(threshold:Double) extends RichFlatMapFunction[SensorReading, (String, Double, Double)]{
  ...
  override def flatMap(in: SensorReading, collector: Collector[(String, Double, Double)]): Unit = {
    ...
    if(diff > threshold){
      collector.collect((in.id,lastTemp,in.timestamp))
    }
